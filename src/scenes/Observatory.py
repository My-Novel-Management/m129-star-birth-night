...
def missing_star_world(w: World):
    ...
    return w.scene("星が消えた世界で",
            ...
            yazaki.do("彼は前髪を切り過ぎて狭い露出してしまった額に手を当て、小さくため息をつく").omit(),
            ...
            yazaki.talk("何度も言ったと思うがね、$chito",
                "星はそんなに簡単には見つからない",
                "世界中の様々な研究者や個人の天文愛好家が星の光を探し続けている",
                "でも誰一人として見つかったという報告を上げられない",
                "国際天文連は地球孤立説を本気で採用しようとしているくらいお手上げ状態なんだ"),
            yazaki.talk("太陽以外の星は全て消え去ってしまった",
                "今や多くの学者がその考えを正しいと認めざるを得ない状況で").omit(),
            ...
            yazaki.do("$Sが得意とするのは物理方面だが数式の話をしても流石にまだ理解はしてもらえないので、",
                    "興味を惹きそうな話題を振るのだけれど、いい加減にそういうネタも無くなりつつある").omit(),
...
def create_star(w: World):
    ...
    return w.scene("星を作ろう",
            ...
            yazaki.talk("天文台は何も星を見つける為だけにあるんじゃないぞ",
                "天体を観測しデータを集めそれらを記録をし、研究成果を社会に還元する",
            ...
            chito.talk("先生までそんな大人だったなんて知らなかったよ！",
                "なんですぐあきらめるなんて言うのさ？",
                "星がないなら作ればいいんだよ！",
...
def chitos_creation(w: World):
    ...
    return w.scene("$chitoの工作",
            ...
            yazaki.do("丘の上に建てられた半球の乗った独特の建物は今、そのスリットを閉じている",
                ...
                "自分がちっぽけなことを気にしすぎているのかも知れないと反省した").omit(),
            ...
            yazaki.do("はい先生、と元気よく返事をした$chitoの前に模造紙を広げると、ノートに手書きしたペットボトルロケットの簡単な設計図を彼に書き写させる",
                ...
                "燃料である水と空気を用いて推進力を得る",
                "ペットボトルは軽くて丈夫でおまけにどこにでもある素材だからこの手の実験には好都合な材料だった"),
            yazaki.do("特に軽いということが重要で、そのお陰で僅かな推進力でも二百メートル程度は打ち上げることができる").omit(),
            chito.talk("痛っ"),
            ...
            chito.do("そう言うと$Sはしょぼんとして「はい」と頷いた"),
            chito.talk("大丈夫だって！", "それより早く次！").omit(),
            chito.do("親指に滲んだ血を舐め取ると、$chitoは真剣な表情で設計図と切断したペットボトルをにらめっこする", "&").omit(),
            yazaki.do("その姿勢に自分の小さい頃を思い出したが小さな吐息を一つして立ち上がると「絆創膏を取ってくる」と言い残して、一旦部屋を出た").omit(),
            )
...
def chito_burned(w: World):
    ...
    return w.scene("$chitoの火傷",
            ...
            dad.talk("おい！"),
            yazaki.do("$chitoの父親だ", "目つきが鋭くて強面だが消防団の団長をやっている町の傑物で人望も厚い"),
            yazaki.talk("どうされました？"),
...
def planetarium(w: World):
    ...
    return w.scene("プラネタリウム",
            ...
            yazaki.explain("自分たちで直せる部分は直して、原板は修理を知古の町工場に出して直してもらうことになった"),
...
def closing_ceremony(w: World):
    ...
    return w.scene("閉館式",
            ...
            w.cmd.change_stage("Observatory"),
            yazaki.be("天文台の閉館式を行う日の朝は昨夜からの雨が続いていたが、",
                "昼過ぎになって晴れ間が見えるようになっていた"),
            nomura.be("#機械整備をしている"),
            shino.be("#準備を手伝っている"),
            yazaki.talk("あ、そこ", "もう少し右側に寄せて"),
            yazaki.do("故障したままだったプラネタリウムを蘇らせるという決断を、最初は$full_shinoも台長も良い顔はしてくれなかったけれど、",
                "最後だからどうしてもやらせて欲しいと頼み込み、何とかゴーサインを出してもらった"),
            yazaki.do("ただ流石に期間が短く、もし一人で全部をやっていたなら今日までには間に合わなかっただろう").omit(),
            shino.talk("主任、原板届きました"),
            yazaki.talk("ありがとう"),
            yazaki.do("速達の小包を$full_shinoから受け取る",
                "恒星原板はプラネタリウムの心臓といってもいい部品だ",
                "直径数センチの小さな円盤に見えない細かい穴が無数に開けられている",
                "この一つ一つがドームに映し出す星の元であり、僅かな穴の大きさで星の等級が決まってしまう",
                "また穴が大きくなるほど投影される星がぼやけてしまう為、シャーペンの芯の数十分の一という精度が求められる"),
            yazaki.do("$nomuraさんたちにも手伝ってもらい、投影機のチェックを行う",
                "原板がセットされると一旦ホールの照明が落とされ、投影レンズを通してドーム天井へと細かな星の像が映し出される",
                "この天文台に設置されているプラネタリウムは光学式と呼ばれるもので、",
                "天球を三十二に分割しそれぞれに対して原板とレンズ、ランプが用意される"),
            shino.talk("いけそうですね"),
            yazaki.do("隣でほっとした表情の$full_shinoの言葉に小さく頷く"),
            dad.come("そこに大きな足音が割って入ってきた"),
            dad.talk("おい！　うちの息子はどこだ？"),
            yazaki.do("$chitoの父親だ"),
            yazaki.talk("どうしたんですか？", "$chito君はここ最近ずっと来てません", "家にも学校にもいないんですか？"),
            dad.talk("学校に行ってないみたいなんだ",
                "今先生たちにも探してもらってるし、消防団の連中にも頼んだ", "パトカーも出てもらってるがまだ何も報告がない"),
            yazaki.do("その様子からはとても嘘や冗談を言っているとは思えない",
                "$Sは$full_shinoに照明を点けてもらって作業中のみんなの手を止めると、事情を説明してこう呼びかけた"),
            yazaki.talk("すみません！",
                "みなさんで$chito君を探すのを手伝ってもらえないでしょうか？"),
            nomura.talk("それは構わないが、先生。こっちはどうするんだ？　準備が間に合わないぞ？"),
            yazaki.talk("そうですね、じゃあ$nomuraさんたちはここをお願いします",
                "$meは$chito君を探してきます"),
            yazaki.go(),
            )
